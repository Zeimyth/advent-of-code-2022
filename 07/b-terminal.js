...
const implementation = (lines) => {
  ...
  const freeSpace = MAX_DRIVE_SIZE - tree.size;
  const spaceNeededToSave = TARGET_FREE_SPACE - freeSpace;

  return findSizeOfDeletionTarget(tree, spaceNeededToSave);
};
...
const findSizeOfDeletionTarget = (folder, spaceNeededToSave) => {
  if (folder.size < spaceNeededToSave) {
    return TARGET_FREE_SPACE;
  } else {
    const childMin = min(folder.folders.map(child => findSizeOfDeletionTarget(child, spaceNeededToSave)));
    return Math.min(childMin, folder.size);
  }
};
...
const min = (arr) => {
  return arr.reduce((soFar, size) => Math.min(soFar, size), TARGET_FREE_SPACE);
}
