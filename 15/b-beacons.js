...
const implementation = (sensors) => {
  for (let i = 0; i < sensors.length; i++) {
    sensors[i].scanDistance = manhattan(sensors[i].sensor, sensors[i].beacon);
  }

  const max = MANUAL_INPUT ? 20 : 4000000;
  ...
  for (let y = 0; y < max; y++) {
    ...
    for (let x = 0; x < max; x++) {
      const coord = {x, y};
      let found = false;
      for (let i = 0; i < sensors.length && !found; i++) {
        const data = sensors[i];
        if (manhattan(data.sensor, coord) <= data.scanDistance) {
          found = true;
          x = data.sensor.x + data.scanDistance - Math.abs(data.sensor.y - y);
        }
      }
      if (!found) {
        return coord.x * max + coord.y;
      }
    }
  }
};

const manhattan = (first, second) => {
