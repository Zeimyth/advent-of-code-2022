...
const implementation = (lines) => {
  ...
  return getEmptySpaces(elves, totalElves);
};
...
const step = (elves) => {
  ...
  const movedElves = moveElves(elves, proposedDirections, potentialEntrants);
  cycleDirections();
  return movedElves;
};
  ...
  for (y of elves.keys()) {
    for (x of elves.get(y).values()) {
      ...
      if (direction === false) {
        ...
        tryMove(movedElves, potentialEntrants, x, y, direction);
      }
    }
  }
